...
class Conv5Small(TFEstimatorBase):
    ...
    def build_model(self):
        ...
        self.model.add(layers.Conv2D(16, 3, strides=(2, 2), input_shape=self.input_shape, activation='relu', padding='same'))
        if self.batch_norm:
            self.model.add(layers.BatchNormalization())

        self.model.add(layers.Conv2D(32, 3, strides=(2, 2), activation='relu', padding='same'))
        if self.batch_norm:
            self.model.add(layers.BatchNormalization())
        ...
        self.model.add(layers.Conv2D(64, 3, strides=(2, 2), activation='relu', padding='same'))
        if self.batch_norm:
            self.model.add(layers.BatchNormalization())
